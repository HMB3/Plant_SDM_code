...
## Loop over directories, species and one threshold for each, also taking a time_slice argument. Next, make the lists generic too
## pervious version in R/old/model_combine.R
combine_gcm_threshold = function(DIR_list, species_list, maxent_path, thresholds, 
                                 percentiles, time_slice, area_occ, write_rasters) {
  ...
  lapply(DIR_list, function(DIR) { 
    ...
    lapply(species_list, function(species) {
      ...
      for (thresh in thresholds) {
        ...
        ## Check if the SUA summary table exists
        SUA_file =   sprintf('%s/%s/full/%s_20%s_%s%s%s.csv', maxent_path,
                             species, species, time_slice, area_occ, "pc_area_SUA_summary_", thresh) 
        ...
        for (percent in percentiles) { 
          ...
          ## The gain/loss raster intersected with the SUAs, instead of the combo_suit_4GCM.................................
          ...
          message('Running SUA area calculations for ', species,
                  ' | 20',  time_slice, ' combined suitability > ', thresh,
          ...
          ## Could be done by rasterizing the SUA shapefile
          #########################################################################################################################
          ## Then, extract the values of the presence raster for each areal unit: generates a list
          ## Could be done by rasterizing the SUA shapefile
          ...
          ## Print the dimensions to screen, this is where the exceptions are
          message(species, ' has ', dim(PERECENT.AREA.CURRENT)[2], ' current columns and ',  dim(PERECENT.AREA.FUTURE)[2], ' future columns')
          ...
          message('For ', species, ' both current and future rasters in SUAs')
          if(dim(PERECENT.AREA.CURRENT)[2] == 3 & dim(PERECENT.AREA.FUTURE)[2] == 3) { 
            
            ## Then name all three columns by NAME!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            setnames(PERECENT.AREA.CURRENT, old = c("name", "1", "0"), new = c("SUA_NAME16", 'Present', 'Absent'))
            setnames(PERECENT.AREA.FUTURE,  old = c("name", "1", "0"), new = c("SUA_NAME16", 'Present', 'Absent'))
            ...
            print(head(PERECENT.AREA.CURRENT))
            print(head(PERECENT.AREA.FUTURE))
            ...
            GCM.AREA.SUMMARY <- data.frame(SUA          = areal_unit$SUA_NAME16, 
                                           AREA_SQKM    = areal_unit$AREASQKM16,
                                           SPECIES      = species,
                                           PERIOD       = time_slice,
            ...
            ## Absent - not there in either
            ## Stable  - exatcly equal. Otherwise this is just present in both
            
            ## We define presnece/absence continuosly within each SUA, not using a binary raster
            ## ..................................................................................
            
            ## So the stable category disappears, unless the area of the SUA occupied is excatly equal
            GCM.AREA.SUMMARY$GAIN_LOSS  <- with(GCM.AREA.SUMMARY, ifelse(
              GCM.AREA.SUMMARY$AREA_CHANGE < 0, 'LOSS', ifelse(
                GCM.AREA.SUMMARY$AREA_CHANGE > 0, 'GAIN', ifelse(
                  GCM.AREA.SUMMARY$CURRENT_AREA == 0 & GCM.AREA.SUMMARY$CURRENT_AREA == 0, 'ABSENCE', ifelse(
                    GCM.AREA.SUMMARY$CURRENT_AREA > 0 & GCM.AREA.SUMMARY$CURRENT_AREA == 0, 'STABLE', 'GAIN')))))
            ...
            message('For ', species, ' neither current nor future rasters are in the SUAs')
            if(dim(PERECENT.AREA.CURRENT)[2] == 2 & dim(PERECENT.AREA.FUTURE)[2] == 2) { 
              ...
              setnames(PERECENT.AREA.CURRENT, old = c("name", "1", "0"), new = c("SUA_NAME16", 'Present', 'Absent'))
              setnames(PERECENT.AREA.FUTURE,  old = c("name", "1", "0"), new = c("SUA_NAME16", 'Present', 'Absent')) ## 0 length for some spp         
              ...
              if(dim(PERECENT.AREA.CURRENT)[2] == 2 & dim(PERECENT.AREA.FUTURE)[2] == 3) {
                message('For ', species, ' current raster not in SUAs, but future raster is')
                ...
                message('For ', species, 'current raster in SUAs, but future raster is not')
                PERECENT.AREA.FUTURE$Present = 0
...
## Loop over directories, species and one threshold for each, also taking a time_slice argument. Next, make the lists generic too
## pervious version in R/old/model_combine.R
SUA_tables = function(DIR_list, species_list, maxent_path, thresholds, time_slice, area_occ) {
  ...
  lapply(DIR_list, function(DIR) { 
    ...
    lapply(species_list, function(species) {
      ...
      for (thresh in thresholds) {
        ...
        ## Use the gain/loss rater to extract per SUA, not the current_suit_thresh...............................................
        ...
        #########################################################################################################################
        ## Then, extract the values of the presence raster for each areal unit: generates a list
        message ("AT EXTRACT 1")
        ext.current  <- raster::extract(current_suit_thresh, areal_unit, method = 'simple')
        
        message ("AT EXTRACT 2")
        ext.future   <- raster::extract(combo_suit_4GCM,     areal_unit, method = 'simple')
        ...
        #########################################################################################################################
        ## Now mutate the current table
        message ("Calculating current SUA area")
        PERECENT.AREA.CURRENT <- tabs.current %>%
        ...
        #########################################################################################################################
        ## Now mutate the future table
        message ("Calculating future SUA area")
        PERECENT.AREA.FUTURE <- tabs.future %>%
        ...
        ## Loss (ie suitable now but not in future)
        ## Stable (suitable in both)
        message ("Calculating gain/loss/stable")
