...
env.world.grids = stack(world.grids.current, PET, AI)
COMBO.RASTER    <- raster::extract(env.world.grids , COMBO.POINTS) %>% 
...
## AI was multiplied by 10k, so divide by 10k
COMBO.RASTER.CONVERT[, (env.variables [c(21)]) := lapply(.SD, function(x) 
  x / 10000 ), .SDcols = env.variables [c(21)]]
...
summary(COMBO.RASTER.CONVERT$AI)
...
if(save_data == "TRUE") {
  ...
  write.csv(COMBO.RASTER.CONVERT, paste0(DATA_path, 'COMBO_RASTER_CONTEXT_',  save_run, '.csv'), row.names = FALSE)
