...
######################################################
## Run GAMs of species gains vs MAXT for 2030, ALL SUA
ALL.GAIN.2030.GAM = gam(SPECIES_GAIN ~ s (CURRENT_MAXT, k = 3), 
...
fig3A<-ggplot(allSUAsGAIN, aes(x=CURRENT_MAXT, y=SPECIES_GAIN, color=factor(PERIOD))) +
...
## Run GAMs of species gains vs MAXT for 2070, BIG SUA
...
#########################################################################################################################
## Run GAMs of species losses vs MAXT for 2070, ALL SUA
...
dev.new(width=17, height=13)
fig3C<-ggplot(allSUAsLOSS, aes(x=CURRENT_MAXT, y=SPECIES_LOSS, color=factor(PERIOD))) +
  ...
  xlab(bquote('Current MAXT of SUA (1960-1990)')) + ylim(0,80) + ylab(bquote('Range loss (%)')) + 
  theme(text = element_text(size=30)) + 
  scale_color_manual(values=c("darkgrey", "black"), name = "Period") + 
...
#########################################################################################################################
## Run GAMs of species losses vs MAXT for 2070, BIG SUA
BIG.LOSS.2070.GAM = gam(SPECIES_LOSS ~ s (CURRENT_MAXT, k = 3), 
                        data = bigSUAsLOSS2070, 
                        method = "REML")
summary(BIG.LOSS.2070.GAM)[["dev.expl"]][1]               
######################################################
## Run GAMs of species losses vs MAXT for 2030, BIG SUA
BIG.LOSS.2030.GAM = gam(SPECIES_LOSS ~ s (CURRENT_MAXT, k = 3), 
...
dev.new(width=17, height=13)
fig3D<-ggplot(bigSUAsLOSS, aes(x=CURRENT_MAXT, y=SPECIES_LOSS, color=factor(PERIOD))) +
  ...
  xlab(bquote('Current MAXT of SUA (1960-1990)')) + ylim(0,80) + 
...
ggsave(filename = "./output/figures/FIG_4/FIG_4_MAXT.png", width = 20, height = 18, plot = fig3, dpi = 300)
