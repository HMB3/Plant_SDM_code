...
#########################################################################################################################
## 2). CREATE A RANDOM SUB-SAMPLE OF X% OF SPP RECORDS, STRATIFIED BY RAINFALL, THEn RUN SDMs AGAIN FOR SELECTED TAXA
#########################################################################################################################


## Use the spatially rarefy occurrence data tool addresses this issue by spatially filtering locality data by a user input 
## distance, reducing occurrence localities to a single point within the specified Euclidian distance.  This tool also allows 
## users to spatially rarefy their data at several distances according to habitat, topographic or climate heterogeneity (Table 1d). 
## For example, occurrence localities could be spatially filtered at 5 km2, 10 km2 and 30 km2 in areas of high, medium and 
## low environmental heterogeneity, respectively. This graduated filtering method is particular useful for studies with l
## imited occurrence points and can maximize the number of spatially independent localities. Three additional tools calculate 
## climate and topographic heterogeneity for rarefying at multiple distances. The climate heterogeneity tool calculates the 
## first three principal components (PCs) of all input climate data. Then it calculates the heterogeneity of each PC, weighs 
## each PC heterogeneity layer by the amount of variation explained, and sums them to produce the final heterogeneity layer
## file:///C:/Users/user/AppData/Roaming/ESRI/Desktop10.6/ArcToolbox/Dlg/MdToolHelp.htm
...
#########################################################################################################################
## This file was created in ArcMap using the settings: 10km, 5 classes, 25km, 5km
BIAS.DF       = readRDS('data/base/HIA_LIST/COMBO/BIAS_SPP.rds')
class(BIAS.DF)
...
dropList <- c("AUS_STA", "AUS_RN_", "RASTERVALU", "coords.x1", "coords.x2")
BIAS.DF  <- BIAS.DF[, !colnames(BIAS.DF) %in% dropList]
names(BIAS.DF)
...
BIAS.DF      = SpatialPointsDataFrame(coords      = BIAS.DF[c("LON", "LAT")],
                                      data        = BIAS.DF,
                                      proj4string = CRS.WGS.84)
BIAS.DF      = spTransform(BIAS.DF, CRS('+init=ESRI:54009'))
projection(BIAS.DF)
length(unique(BIAS.DF$searchTaxon))
names(BIAS.DF)
