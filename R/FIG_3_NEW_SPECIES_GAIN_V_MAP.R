...
ALL.GAIN.2070.GAM = gam(Species.gain ~ s (MAP, k = 3), 
                        data = allSUAs2070, 
                        method = "REML")
...
ALL.GAIN.2030.GAM = gam(Species.gain ~ s (MAP, k = 3), 
                        data = allSUAs2030, 
                        method = "REML")
...
fig2A<-ggplot(allSUAs, aes(x=MAP, y=Species.gain, color=factor(PERIOD))) +
...
BIG.GAIN.2070.GAM = gam(Species.gain ~ s (MAP, k = 3), 
                        data = bigSUAs2070, 
                        method = "REML")
...
BIG.GAIN.2030.GAM = gam(Species.gain ~ s (MAP, k = 3), 
                        data = bigSUAs2030, 
                        method = "REML")
...
##
fig2B<-ggplot(bigSUAs, aes(x=MAP, y=Species.gain, color=factor(PERIOD))) +
...
ALL.LOSS.2070.GAM = gam(Species.loss ~ s (MAP, k = 3), 
                        data = allSUAs2070, 
                        method = "REML")
...
ALL.LOSS.2030.GAM = gam(Species.loss ~ s (MAP, k = 3), 
                        data = allSUAs2030, 
                        method = "REML")
...
## 
dev.new(width=17, height=13)
fig2C<-ggplot(allSUAs, aes(x=MAP, y=Species.loss, color=factor(PERIOD))) +
  ...
  geom_smooth(method="gam", formula = y ~ s(x, k = 3), se=FALSE, fullrange=TRUE, size = 2) +
  xlab(bquote('Current MAP of SUA (1960-1990)')) + ylim(0,80) + ylab(bquote('Species lost (%)')) + 
  theme(text = element_text(size=30)) + 
  scale_color_manual(values=c("darkgrey", "black"), name = "Period") + 
...
BIG.LOSS.2070.GAM = gam(Species.loss ~ s (MAP, k = 3), 
                        data = bigSUAs2070, 
                        method = "REML")
...
BIG.LOSS.2030.GAM = gam(Species.loss ~ s (MAP, k = 3), 
                        data = bigSUAs2030, 
                        method = "REML")
...
fig2D<-ggplot(bigSUAs, aes(x=MAP, y=Species.loss, color=factor(PERIOD))) +
...
ggsave(filename = "./output/figures/FIG_3/FIG_3_NEW_SUA_SPECIES_MAP.png", width = 20, height = 18, plot = fig2, dpi = 300)
