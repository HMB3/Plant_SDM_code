...
env.grids.2050 = lapply(scen, function(x) {
  
  ## Assign the scenario name (to use later in the plot)
  scen_name = gcms$GCM[gcms$id == x]
  ...
  lapply(species_list, function(species) {
    ...
    if(!file.exists(sprintf('F:/green_cities_sdm/output/maxent/STD_VAR_ALL/%s/full/%s_%s.tif',
      ...
      if(!file.exists(f_future)) {
        ...
        ## Use the levelplot function to make a multipanel output: occurrence points, current raster and future r
