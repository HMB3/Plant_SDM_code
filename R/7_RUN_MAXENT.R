...
#########################################################################################################################
## 3). TABULATE MAXENT STATISTICS - ADD NICHE DATA LATER
#########################################################################################################################


## Code that adds niche info is now in './R/COLLATE_MAXENT_RESULTS.R' 
## This code is just for running on Katana
## Print the species run to the screen
...
MAXENT.RESULTS <- maxent.tables %>%         
  ...
  lapply(function(x) {
    ...
    if (grepl("BS", results_dir)) {
      m = readRDS(sprintf('%s/%s/full/model.rds', results_dir, x))
...
## This is used for combining the rasters
