...
#########################################################################################################################
## 2). MAP SPECIES NAMES BACK ONTO ALA DATA
#########################################################################################################################
...
#########################################################################################################################
## Use "Taxonstand" to check the taxonomy. However, this also assumes that the ALA data is clean
ALA.PLANT.TAXO <- TPL(unique(ALA.TRIM$scientificName), infra = TRUE,
                 corr = TRUE, repeats = 100)  ## to stop it timing out...
sort(names(ALA.PLANT.TAXO))
...
ALA.PLANT.TRIM <- ALA.PLANT.TRIM %>%
  left_join(., ALA.PLANT.TAXO, by = c("scientificName" = "Taxon"))
...
ALA.PLANT.TRIM$scientificName  = trimws(ALA.PLANT.TRIM$scientificName)


## Then combine the genus and species returned by TPL into
ALA.PLANT.TRIM$TPL_binomial  = with(ALA.PLANT.TRIM, paste(New.Genus, New.Species, sep = " "))


## Now match the searchTaxon with the binomial returned by TPL :: this would be the best field to filter on
ALA.PLANT.TRIM$taxo_agree <- ifelse(
  ALA.PLANT.TRIM$scientificName == ALA.PLANT.TRIM$TPL_binomial, TRUE, FALSE)


## How many species agree?
round(with(ALA.PLANT.TRIM, table(taxo_agree)/sum(table(taxo_agree))*100), 1)
round(with(ALA.PLANT.TRIM, table(New.Taxonomic.status)/sum(table(New.Taxonomic.status))*100), 1)


## Also keep the unresolved records:
ALA.PLANT.UNRESOLVED <- ALA.PLANT.TRIM %>%
...
## Also keep the unresolved records:
ALA.PLANT.RESOLVED <- ALA.PLANT.TRIM %>%
...
## Also keep the unresolved records:
ALA.PLANT.TAXO.DISAGREE <- ALA.PLANT.TRIM %>%
...
head(ALA.PLANT.UNRESOLVED, 50)[, c("scientificName", 
                             "TPL_binomial",
                             "taxo_agree",
                             "New.Taxonomic.status")]


head(ALA.PLANT.RESOLVED, 50)[, c("scientificName", 
                           "TPL_binomial",
                           "taxo_agree",
                           "New.Taxonomic.status")]


head(ALA.PLANT.TAXO.DISAGREE, 50)[, c("scientificName", 
                                "TPL_binomial",
                                "taxo_agree",
                                "New.Taxonomic.status")]
...
unique(ALA.PLANT.UNRESOLVED$New.Taxonomic.status)
dim(ALA.PLANT.UNRESOLVED)   ## 1.2 million unresolved records, quite a lot!
...
ALA.PLANT.TRIM.TAXO = ALA.PLANT.TRIM
ALA.PLANT.PROBLEMS <- with(ALA.PLANT.TRIM.TAXO,
...
ALA.PLANT.CLEAN <- ALA.PLANT.TRIM.TAXO %>% 
...
xy <- cellFromXY(world.temp, ALA.PLANT.CLEAN[c("lon", "lat")]) %>% 
...
ALA.PLANT.LAND = filter(ALA.PLANT.CLEAN, cellFromXY(world.temp, ALA.PLANT.CLEAN[c("lon", "lat")]) %in% 
                     unique(cellFromXY(world.temp, ALA.PLANT.CLEAN[c("lon", "lat")]))[onland])


## how many records were on land?
records.ocean = dim(ALA.PLANT.CLEAN)[1] - dim(ALA.PLANT.LAND)[1]  ## 91575 records are in the ocean   


## Print the dataframe dimensions to screen
dim(ALA.PLANT.LAND)
length(unique(ALA.PLANT.LAND$searchTaxon))
